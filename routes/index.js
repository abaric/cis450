...
router.get('/crime', function(req, res, next) {
  ...
  connection.query('SELECT n.id, COUNT(h.crime_id) as crimes FROM neighborhoods n, happened_in h, crimes c WHERE n.id = h.neighborhood_id AND c.id = h.crime_id AND c.type LIKE ' + "'" + field + "'" + ' GROUP BY n.id ORDER BY crimes ' + order + ' LIMIT ' + rows, function(err, rows, fields) {
    if (err) {
      console.log(err);
      res.send(500);
    }
